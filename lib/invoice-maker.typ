...
#let languages = (
  ...
  zh: (
    ...
    reverse-charge: "Reverse Charge",
    total: "Total",
    due-text: val => [请于*#val*前将款项转入以下银行账户：],
    owner: "Owner",
    iban: "IBAN",
  ),
)
...
#let invoice(
  ...
  discount: none,
  vat: 0.19,
  vat-always: true, // Always charge VAT (even if reverse charge applies)
  data: none,
  override-translation: none,
  // doc,
) = {
  // Set styling defaults
  styling.font = styling.at("font", default: "Microsoft YaHei")
  ...
  // Translations
  let t = if type(language) == str { languages.at(language) } else if (
  ...
  let signature = ""
  let issuing-date = if issuing-date != none { issuing-date } else {
    ...
    #text(weight: "bold", size: 2em)[
      #(
        if title != none { title } else {
          if cancellation-id != none { t.cancellation-invoice } else {
            t.invoice
          }
        }
      )
  ...
  let delivery-date = if delivery-date != none { delivery-date } else { TODO }
  ...
  box(height: 12em)[
    #columns(2, gutter: 4em)[
      #set heading(level: 3, outlined: false, numbering: none, supplement: none)
      === #t.recipient
      // #t.recipient
      #v(0.5em)
      #recipient.name \
  ...
  let discount-value = if discount == none { 0 } else {
    if (discount.type == "fixed") { discount.value } else if (
      discount.type == "proportionate"
    ) {
      sub-total * discount.value
    } else { panic(["#discount.type" is no valid discount type]) }
  }
  let discount-label = if discount == none { 0 } else {
    if (discount.type == "fixed") {
      str(discount.value) + " " + currency
  ...
  if cancellation-id == none {
    let due-date = if due-date != none { due-date } else {
      (parse-date(issuing-date) + duration(days: 14)).display(
        "[year]-[month]-[day]",
      )
    }
